...
fprintf(['\nInterpretation: S is weakly constrained (%d non-slack overvoltage violations, all mild,\n' ...
    'phase-b only; no undervoltage); S'' shows how DER raises the whole voltage profile (Vmin up,\n' ...
    'existing phase-b overvoltage slightly worse, no new violation class); S'''' (lambda=%.2f) is a\n' ...
    'HEAVILY stressed regime (%d/%d non-slack terminals in violation) with opposing pressures --\n' ...
    'undervoltage on phases a/c, overvoltage still on phase b. Whether this is "recoverable" by the\n' ...
    'four community participants'' actual capability sets is a separate feasibility question, not\n' ...
    'yet tested here -- do not assume it from the violation count alone.\n'], ...
    S.rep_ac_ns.n_overvoltage, lambda, Spp.rep_ac_ns.n_violations, Spp.rep_ac_ns.n_physical);
...
% CRITICAL: the tangent plane feeding the exported interface must be built
% at S'''s NONLINEAR AC-CONVERGED operating point, not the one-shot manifold
% estimate (Spp.tp) -- that estimate is kept only for the manifold-vs-AC
% accuracy comparison above. "Adopting S'' as the operating system" (spec
% Section 3) means adopting its validated (AC) state as v*''.
tp_ac = bd_tangent_general(Y, n, Spp.ac.v, Spp.ac.theta);

sh = derive_shared_constraint(tp_ac, Spp.p, Spp.q, n, valid_mask, vmin, vmax, der_meta.community);

fprintf('\n============================================================\n');
fprintf('DSO-TO-COMMUNITY INTERFACE (derived from S'''' nonlinear AC operating point)\n');
fprintf('============================================================\n');
...
fprintf('rows with margin < 0   : upper %d, lower %d  (violated at x_F=0, i.e. at the S'''' baseline)\n', ...
    nnz(sh.margin_upper < 0), nnz(sh.margin_lower < 0));

% --- Invariant check: exported b_F_sh MUST reproduce the independently
% computed AC ground-truth violation pattern exactly. This is the specific
% regression that shipped a wrongly-anchored interface once already
% (anchored at the one-shot linear point, with a spurious RHS subtraction)
% -- refuse to export if it recurs.
assert(all(size(sh.A_F_sh) == [2*sh.nPhys, sum(sh.n_physical)]), ...
    'Unexpected A_F_sh dimensions.');

expected_b = [vmax - sh.vstar_phys; sh.vstar_phys - vmin];
assert(norm(sh.b_F_sh - expected_b, inf) < 1e-9, ...
    'run_ieee13_pipeline:badExport', ...
    'b_F_sh does not match [vmax-v*''''; v*''''-vmin] to tolerance -- refusing export.');

n_over_ac  = Spp.rep_ac_ns.n_overvoltage;
n_under_ac = Spp.rep_ac_ns.n_undervoltage;
n_over_sh  = nnz(sh.b_F_sh(1:sh.nPhys) < 0);
n_under_sh = nnz(sh.b_F_sh(sh.nPhys+1:end) < 0);
fprintf('Invariant check: exported violations upper/lower = %d/%d, AC ground truth = %d/%d\n', ...
    n_over_sh, n_under_sh, n_over_ac, n_under_ac);
assert(n_over_sh == n_over_ac && n_under_sh == n_under_ac, ...
    'run_ieee13_pipeline:badExport', ...
    ['Exported constraint''s violation pattern (%d over / %d under) does not match the ' ...
     'independently computed nonlinear-AC ground truth (%d over / %d under) -- refusing export.'], ...
    n_over_sh, n_under_sh, n_over_ac, n_under_ac);
