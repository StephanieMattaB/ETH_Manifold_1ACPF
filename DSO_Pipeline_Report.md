...
### 4.1 Topology and phase-resolved views

![Topology map: worst-phase voltage per bus, S vs S' vs S'' (nonlinear AC)](figures/fig7_topology_map.png)

The same progression at a glance: $\mathcal S$'s tree is mostly warm
(near/above 1.05, phase-b driven) with two cool leaves (611, 652);
$\mathcal S'$ pushes further warm feeder-wide; $\mathcal S''$ flips almost
the entire tree cold (below 0.95) except the source-adjacent 650/632 and
the chronically warm phase-b-dominated 634 branch, which stays warm
throughout. Community nodes (645, 611, 652, 671) are ringed.

![Magnitude and angle, one-shot linear vs nonlinear AC, all 3 phases, S''](figures/fig8_magnitude_angle_S2.png)

This is the first figure to show $\theta$ rather than only $|V|$: the
one-shot linear model tracks angles closely across all three phases (the
`o`/`*` markers nearly coincide in the right column) even at $\mathcal
S''$'s stress level — the 1.93% error in Section 4's table is concentrated
almost entirely in the magnitude, not the angle.
...
confirmed rather than assumed. This is visible directly, not just in
aggregate statistics, by sweeping a single terminal's own $(p,q)$ around
the $\mathcal S''$ anchor and re-solving the true nonlinear AC model at
every grid point (675.c, the worst terminal, $\pm0.06$ p.u. — comparable to
the local capability box):

![True manifold vs local tangent plane at 675.c, and the resulting error surface](figures/fig9_manifold_675c.png)

The true manifold (left, mesh) visibly curves away from the flat tangent
plane (red) anchored at $\mathcal S''$; the resulting error (right) is the
textbook quadratic bowl, exactly zero at the anchor and growing outward in
every direction — max $1.6\times10^{-2}$ p.u. at the sweep's far corner,
consistent with the $e_{max}$ range in the table above. Within the assumed capability box under
...
octave --no-gui --eval "generate_topology_figures"     % Figure 7
octave --no-gui --eval "generate_angle_magnitude_figure"  % Figure 8
octave --no-gui --eval "generate_manifold_illustration"   % Figure 9
```
...
| `ieee13_topology_layout.m`, `diverging_color.m`, `generate_topology_figures.m` | Figure 7 |
| `generate_angle_magnitude_figure.m` | Figure 8 |
| `generate_manifold_illustration.m` | Figure 9 |
